...
def test_a_entrada_da_execucao_carrega_equacao_e_meta(sfn):
    ...
    entrada = next(iter(sfn.executions.values())).input
...
def test_um_lote_vira_varias_execucoes(sfn):
    call(
        *[
            record(message_id=f"m{i}", body=f'{{"a": 1, "b": {i}, "c": 1}}')
            for i in range(10)
        ]
    )
...
def test_corpo_que_e_lista_vira_raw_body(sfn):
    ...
    assert next(iter(sfn.executions.values())).input["equation"] is None


def test_dois_corpos_invalidos_diferentes_nao_colidem(sfn):
    call(
        record(message_id="a", body="{quebrado 1"),
        record(message_id="b", body="{quebrado 2"),
    )
...
def test_o_caos_do_atributo_entra_no_meta(sfn):
    ...
    assert next(iter(sfn.executions.values())).input["meta"]["chaos"] == {
        "state": "Delta",
        "fails": 2,
    }
...
def test_caos_invalido_e_ignorado_sem_derrubar_a_mensagem(sfn):
    registro = record()
    registro["messageAttributes"]["Chaos"] = {
        "stringValue": "{quebrado",
        "dataType": "String",
    }

    call(registro)

    assert "chaos" not in next(iter(sfn.executions.values())).input["meta"]
...
def test_atributo_em_maiuscula_tambem_e_lido(sfn):
    ...
    registro["messageAttributes"]["BatchId"] = {
        "StringValue": "b9",
        "DataType": "String",
    }
    ...
    assert next(iter(sfn.executions.values())).input["meta"]["batch_id"] == "b9"
...
def test_falha_inesperada_vira_retry_so_daquela_mensagem(monkeypatch, sfn):
    ...
    resultado = call(
        record(message_id="ruim"), record(message_id="bom", body='{"a":2,"b":4,"c":1}')
    )
...
def test_uma_falha_nao_derruba_o_lote_inteiro(monkeypatch, sfn):
    ...
    resultado = call(
        record(message_id="a"), record(message_id="b", body='{"a":2,"b":4,"c":1}')
    )
...
def test_log_conta_iniciadas_e_duplicadas(capsys):
    ...
    resumo = next(linha for linha in linhas if linha["event"] == "batch_dispatched")
